...
import { BOARD_COLS, BOARD_ROWS, randomTetromino } from '../../shared/tetrominos'
import {
  createEmptyBoard,
  rotateCW,
  isColliding,
...
import {
  soundRotate, soundLand,
  soundClear1, soundClearMulti, soundTetris,
  soundGameOver, soundFlip, soundLevelUp,
} from '../../shared/sound'
...
function clearLinesGravity(board: Board, lines: number[], gravity: Gravity): Board {
  const lineSet = new Set(lines)
  const remaining = board.filter((_, i) => !lineSet.has(i))
  const empty = Array.from({ length: lines.length }, () =>
    Array.from({ length: BOARD_COLS }, () => ({ kind: 'empty' as const, color: '' }))
  )
  return gravity === 'down' ? [...empty, ...remaining] : [...remaining, ...empty]
}
...
function compactBoard(board: Board, gravity: Gravity): Board {
  ...
  return gravity === 'down' ? [...emptyRows, ...filledRows] : [...filledRows, ...emptyRows]
}
...
function createPiece(gravity: Gravity): Piece {
  ...
  return { shape: tmpl.shape, color: tmpl.color, x: spawnX(tmpl.shape), y: spawnY(tmpl.shape, gravity) }
}
...
export function useGravityTetris() {
  ...
  const dropTick = useCallback(() => {
    ...
    setState(prev => {
      ...
      const cleared = fullLines.length ? clearLinesGravity(merged, fullLines, prev.gravity) : merged
      ...
      // 天井到達 → アウト
      if (isCeilingReached(cleared, prev.gravity)) {
        soundGameOver()
        gameOverRef.current = true
        return {
          ...prev, board: cleared, piece: null, score: newScore, lines: newLines,
          level: newLevel, gameOver: true, isTopScore: isTopScore('gravity', newScore),
          flashingRows: fullLines, levelUpFlash: false,
        }
      }
    ...
    setTimeout(() => {
      ...
      setState(prev => {
        ...
        if (!isColliding(prev.board, spawnPiece)) {
          return { ...prev, piece: spawnPiece, nextPiece: newNextPiece, isFlipping: false }
        }
        gameOverRef.current = true
        return { ...prev, piece: null, isFlipping: false, gameOver: true }
      })
      ...
      setState(prev => {
        ...
        switch (e.key) {
          case ' ': {
            ...
            soundRotate(); return { ...prev, piece: r }
          }
          case 'ArrowUp': {
            const dy = prev.gravity === 'down' ? 1 : -1
